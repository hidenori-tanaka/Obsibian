...
まず知っておくべきことは、**利用できる割り込み優先度の総数は、実装依存である**という点です。  
つまり、その数は ARM Cortex-M コアを使用する **マイクロコントローラメーカーの判断によって決まります**。  
その結果、すべての ARM Cortex-M マイクロコントローラが同じ数の割り込み優先度を持つわけではありません。

ARM Cortex-M アーキテクチャ自体は **最大 256 種類の優先度** を扱える仕様になっています（優先度ビット数は最大 8 ビットで、0〜0xFF の範囲が使用可能）。  
しかし、ARM Cortex-M コアを採用している大半のマイクロコントローラは、これらすべてにはアクセスできず、**その一部のみを実装**しています。

例えば：

- **TI Stellaris Cortex-M3 / Cortex-M4** マイクロコントローラ  
    → **3 ビットの優先度**を実装  
    → **8 種類のユニークな優先度値**が利用可能
    
- **NXP LPC17xx（Cortex-M3）** マイクロコントローラ  
    → **5 ビットの優先度**を実装  
    → **32 種類のユニークな優先度値**が利用可能
    

---

もしあなたのプロジェクトで **CMSIS ライブラリのヘッダファイル**を使用している場合は、  
`__NVIC_PRIO_BITS` の定義を見ることで、**利用可能な優先度ビット数**を確認できます。
...
FreeRTOSConfig.h にある **`configMAX_SYSCALL_INTERRUPT_PRIORITY`** は、  
この **2つのグループを分ける境界となる優先度** を定義します。

この設定に適切な値は、  
**マイクロコントローラで実装されている優先度ビット数によって異なります。**

### Preempt Priority and Subpriority

### Cortex-M hardware details

8ビットの優先度レジスタは **「プリエンプト優先度（preempt priority）」** と  
**「サブ優先度（sub priority）」** の2つに分割されます。  
...
すべての優先度ビットを **プリエンプト優先度ビットに割り当て**,  
**サブ優先度ビットを 0 にする**ことが推奨されています。
