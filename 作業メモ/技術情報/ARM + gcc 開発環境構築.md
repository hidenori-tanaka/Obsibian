...
[ARM のサイト](https://developer.arm.com/downloads/-/gnu-rm)から環境に合ったインストーラーをダウンロード＆インストールしたら完了

開発環境にはmakeなどのUnix系ツールが必要となる。
自分の開発環境ではWindowsから直接コマンド実行可能なMinGW(MSYS)で統一する。
極力ツール類はMSYSでインストールして使えるようにする。(Linuxで作業する場合にも同じ手順でできるため)                                 
[[MSYS2のインストール]]
# GCCオプション
...
https://blueeyes.sakura.ne.jp/2018/10/31/1676/

- L28 `#pragma clang system_header`
	clangの場合にこのファイルをシステムヘッダとして扱うというプラグマ。
	- そのヘッダ内の警告が抑制される
	- MISRAや-Wall/-Wextraなどの警告対象から場外される
	- そのヘッダ内で出る警告は通常表示されなくなる
-  L65 CMSISのバージョンを表すマクロを生成
-  L162 `cmsis_compiler.h` をインクルード　
